      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10" ref={chartRef} onMouseMove={handleMouseMove}>
                  ...
                  return (
                    <g key={`orbit-group-${p.body}`}>
                      ...
                      <circle 
                        ...
                        strokeWidth={isActive || isOrbitHovered ? "2.5" : "0.5"}
        ...
        {(hoveredSign || hoveredOrbit) && (
            ...
            >
                ...
                {/* NEW ORBIT TOOLTIP LOGIC */}
                {hoveredOrbit && (() => {
                    const p = planetCoords.find(p => p.body === hoveredOrbit);
                    if (!p) return null;
                    const elName = p.element || ZODIAC_ELEMENTS[p.sign] || 'Unknown';
                    const elColor = ELEMENT_COLORS[elName] || '#fff';

                    return (
                        <>
                             <div className="flex items-center gap-2">
                                 <span className="w-2 h-2 rounded-full shadow-[0_0_8px_currentColor]" style={{ color: elColor, backgroundColor: elColor }}></span>
                                 <span className="font-serif text-lg text-white">{p.body} Orbit</span>
                             </div>
