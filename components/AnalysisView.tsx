...
export const AnalysisView: React.FC<Props> = ({ result, state, onGenerateImage, transits }) => {
  ...
  // Auto-generate symbol when calculation completes
  const hasAutoGenerated = useRef(false);

  useEffect(() => {
    if (state === CalculationState.COMPLETE && !hasAutoGenerated.current) {
      hasAutoGenerated.current = true;
      // Trigger auto-generation with default balanced config after a short delay
      setTimeout(() => {
        onGenerateImage(defaultConfig);
      }, 1500); // 1.5 second delay to let user see results first
    }
  }, [state, onGenerateImage, defaultConfig]);
